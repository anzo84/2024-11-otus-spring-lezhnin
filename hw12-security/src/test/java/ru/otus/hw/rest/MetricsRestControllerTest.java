...
class MetricsRestControllerTest {
    ...
    void shouldGetMetricsAndReturnOk() throws Exception {
        ...
        mockMvc.perform(get("/api/metrics")
            ...
            .andExpect(jsonPath("$[0].name").value("authorCount"))
            .andExpect(jsonPath("$[0].value").value(10L))
            .andExpect(jsonPath("$[1].name").value("bookCount"))
            .andExpect(jsonPath("$[1].value").value(20L))
            .andExpect(jsonPath("$[2].name").value("genreCount"))
            .andExpect(jsonPath("$[2].value").value(30L))
            .andExpect(jsonPath("$[3].name").value("commentCount"))
            .andExpect(jsonPath("$[3].value").value(40L));
    }
}